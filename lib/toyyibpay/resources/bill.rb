  ...
  module Resources
    ...
    class Bill < APIResource
      ...
      def create(params = {})
        ...
        # PCI-DSS: Ensure no card data is being sent
        ToyyibPay::Compliance.validate_no_card_data(params)

        # Sanitize and validate input parameters
        params = ToyyibPay::Util.sanitize_bill_params(params)

        # Validate callback URL security (HTTPS required)
        if params[:billCallbackUrl]
          ToyyibPay::Compliance.validate_callback_url(params[:billCallbackUrl])
        end

        # Ensure categoryCode is set
        params[:categoryCode] ||= client.config.category_code

        response = request(:post, "/index.php/api/createBill", params)

        # Audit log the bill creation (PCI-DSS Requirement 10)
        ToyyibPay::AuditLogger.log_bill_created(
          bill_code: response&.first&.dig("BillCode"),
          amount: params[:billAmount],
          user_id: params[:_audit_user_id], # Internal param, not sent to API
          ip_address: params[:_audit_ip_address]
        )
